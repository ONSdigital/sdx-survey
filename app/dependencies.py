...
def get_deliver_service(
    settings: Settings = Depends(get_settings), http: HttpService = Depends(get_http_service)
) -> DeliverService:
    return DeliverService(settings.deliver_service_url, http)


def get_receipt_service(
    settings: Settings = Depends(get_settings), publish: PubsubService = Depends(get_pubsub_service)
) -> ReceiptService:
    return ReceiptService(settings, publish)


def get_transform_poster(
    settings: Settings = Depends(get_settings), http_service: HttpService = Depends(get_http_service)
) -> TransformPoster:
    return TransformPoster(transformer_url=settings.transformer_service_url, http_service=http_service)


def get_image_poster(
    settings: Settings = Depends(get_settings), http_service: HttpService = Depends(get_http_service)
) -> ImagePoster:
    return ImagePoster(image_url=settings.image_service_url, http_service=http_service)


def get_transform_selector(
    transform_poster: TransformPosterBase = Depends(get_transform_poster),
...
def get_transformer_service(transform_selector: TransformSelector = Depends(get_transform_selector)) -> TransformService:
    return TransformService(transform_selector)


def get_survey_processor(
    transformer_service: TransformerBase = Depends(get_transformer_service),
    deliver_service: DeliverBase = Depends(get_deliver_service),
    receipt_service: ReceiptServiceBase = Depends(get_receipt_service),
    comments_service: CommentsBase = Depends(get_comments_service),
) -> SurveyProcessorV2:
    return SurveyProcessorV2(transformer_service, deliver_service, receipt_service, comments_service)


def get_adhoc_processor(
    transformer_service: TransformerBase = Depends(get_transformer_service),
    deliver_service: DeliverBase = Depends(get_deliver_service),
    receipt_service: ReceiptServiceBase = Depends(get_receipt_service),
    comments_service: CommentsBase = Depends(get_comments_service),
...
def get_survey(
    ...
    feedback_processor: ProcessorBase = Depends(get_feedback_processor),
) -> Survey:
    return Survey(
        settings,
