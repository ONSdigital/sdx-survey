...
class CommentsService(CommentsBase):
    ...
    def store_comments(self, response: Response):
        ...
        comment = Comment(
            transaction_id=transaction_id, kind=kind, encrypted_data=encrypted_data, submitted_time=submitted_time
        )
    ...
    def _commit_to_datastore(self, comment: Comment):
        ...
        data = {"created": comment.created, "encrypted_data": comment.encrypted_data}
        self._comments_writer.commit_entity(
            data,
            kind=comment.kind,
